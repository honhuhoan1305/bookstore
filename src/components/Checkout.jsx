...
const Checkout = () => {
  ...
  var total = 0;
  const itemList = (item) => {
    total = total + item.id * item.quantity;
    return (
  ...
  const handleSubmit = (event) => {
    ...
    fetch('YOUR_API_ENDPOINT', {
      ...
      .then((response) => {
        ...
        if (response.ok) {
          setIsSubmitted(true); // Cập nhật state `isSubmitted` thành true nếu gửi thành công
        } else {
 ...
 <div className="row g-3">
                            ...
                            <button className="w-100 btn btn-primary btn-lg" type="submit">Continue to checkout</button>        </form>
      </div>
    </div>
  </div>
</>
    ...
    return (
        <>
            ...
            <button type="button" className="btn btn-outline-primary ms-auto" data-bs-toggle="modal" data-bs-target="#loginModal">
                <span className="fa fa-sign-in me-1"></span> Checkout
            </button>
